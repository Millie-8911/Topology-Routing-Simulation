...
def generate_forwarding_table_with_range(table):
    ...
    for row in table:
        ...
        if row[0] != '0.0.0.0':
            ...
            # 7. Build the new row..
            # new_row = [network_dst_string, netmask_string, row[2], ip_range[0], ip_range[1]]
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    host = '127.0.0.1'
    # Send packets to Router 3 via port 8003
    router_3_port = 8003
    # Send packets to Router 4 via port 8004
    router_4_port = 8004

    # Create sockets
    router_3_soc = create_socket(host, router_3_port)
    router_4_soc = create_socket(host, router_4_port)
        ...
        while True:
            ...
            for row in forwarding_table_with_range:
                ...
                if min_ip <= destinationIP_int <= max_ip:
                    ...
                    print(f'send_to_router:{send_to_router}')
                    break
...
# Start the server for Router 2 on port 8002
# Listen to port 8002 to get packets from Router 1
thread_8002 = threading.Thread(target=start_server, args=(8002, forwarding_table_with_range, default_gateway_port))
thread_8002.start()
