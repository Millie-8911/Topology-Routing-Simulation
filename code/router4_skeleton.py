...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    router_5_soc = create_socket(host, 8005) # Connection between Router 4 and Router 5
    router_6_soc = create_socket(host, 8006) # Connection between Router 4 and Router 6
...
# Start the server for Router 4 on port 8004
# Listen to port 8004 to get packets from Router 1
thread_8004 = threading.Thread(target=start_server, args=(8004, forwarding_table_with_range, default_gateway_port))
thread_8004.start()
